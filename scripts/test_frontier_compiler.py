...
The block marked UNCERTAINTY METHODOLOGY is the load-bearing one. The compiler
used to fall back to `0.15 x the roster's observed score spread` whenever no
dispersion was published, which turns "these models sit far apart" into "this
gap is real". Those tests exist so the fallback cannot come back.
...
def base_meta(**over) -> dict:
    meta = {
        ...
        "direction_setting_hierarchy": {"order": []},
...
def ci(v):
    return {"kind": "published_ci", "value": v}
...
# =====================================================================
# UNCERTAINTY METHODOLOGY -- what may and may not set a direction
# =====================================================================

# ------------------------- no dispersion + tiny difference -> UNRESOLVED
m = base_meta()
m["comparability_groups"] = {"g": group()}
m["records"] = [row("a", "Opus 5", 50.4), row("b", "Astra", 50.0), row("c", "GPT-5.6 Sol", 40.0)]
c0 = direction(m)["comparisons"][0]
check("no published dispersion + a 0.4 gap -> UNRESOLVED, not a winner",
      c0["direction"], "UNRESOLVED")
check("  ...and the basis says why", c0["band_basis"], "no_published_dispersion")

# ---------- no dispersion + a LARGE gap is still UNRESOLVED (the old fallback)
m = base_meta()
m["comparability_groups"] = {"g": group()}
m["records"] = [row("a", "Fable 5.1", 52.0), row("b", "Astra", 59.0), row("c", "GPT-5.6 Sol", 40.0)]
c0 = direction(m)["comparisons"][0]
check("a 7-point gap across a 19-point spread is STILL UNRESOLVED without an interval",
      c0["direction"], "UNRESOLVED")
check("  ...the spread ratio survives only as a labelled diagnostic",
      c0["gap_over_observed_spread_diagnostic"], round(7.0 / 19.0, 4))

# ------------------- an arbitrary, undeclared band kind cannot set a direction
m = base_meta()
m["comparability_groups"] = {"g": group(band={"kind": "min_fraction_of_spread", "value": 0.15})}
m["records"] = [row("a", "Opus 5", 50.0), row("b", "Astra", 47.0), row("c", "GPT-5.6 Sol", 40.0)]
try:
    C.build(m, "deadbeef")
    check("a fraction-of-spread band is rejected at build time", "no exception", "raised")
except C.SpreadFallbackResurrected:
    check("a fraction-of-spread band is rejected at build time", "raised", "raised")
check("  ...and on its own it sets no direction either",
      direction(m)["comparisons"][0]["direction"], "UNRESOLVED")

# ----------------------------- a resurrected top-level spread rule is rejected
m = base_meta()
m["comparability_groups"] = {"g": group()}
m["direction_setting_hierarchy"] = {"order": [], "min_fraction_of_spread": 0.15}
m["records"] = [row("a", "Opus 5", 50.0), row("b", "Astra", 47.0)]
try:
    C.build(m, "deadbeef")
    check("min_fraction_of_spread cannot be reintroduced", "no exception", "raised")
except C.SpreadFallbackResurrected:
    check("min_fraction_of_spread cannot be reintroduced", "raised", "raised")

# ------------------------------------------- overlapping real uncertainty
m = base_meta()
m["comparability_groups"] = {"g": group(band=ci(5.0))}
...
# --------------------------------------- clearly separated real uncertainty
m = base_meta()
...
# ---------------------- repeated-trial dispersion is accepted, same convention
m = base_meta()
m["comparability_groups"] = {"g": group(band={"kind": "repeated_trial_variance", "value": 1.0})}
m["records"] = [row("a", "Opus 5", 70.0), row("b", "Astra", 73.0), row("c", "GPT-5.6 Sol", 60.0)]
c0 = direction(m)["comparisons"][0]
check("repeated-trial SD 1.0 becomes a 2.0 band", c0["band"], 2.0)
check("  ...so a 3.0 gap does separate them", c0["direction"], "codex")

# ------------- a benchmark owner's own practical-significance bar is accepted
m = base_meta()
m["comparability_groups"] = {"g": group(band={"kind": "owner_practical_threshold", "value": 4.0})}
m["records"] = [row("a", "Opus 5", 70.0), row("b", "Astra", 73.0), row("c", "GPT-5.6 Sol", 60.0)]
c0 = direction(m)["comparisons"][0]
check("an owner-published practical threshold is taken at face value", c0["band"], 4.0)
check("  ...and a 3.0 gap is inside it", c0["direction"], "equivalent")

# ------------------------------- dispersion published for ANOTHER benchmark
m = base_meta()
m["comparability_groups"] = {"g": group(band={"kind": "published_se", "value": 2.0,
...
check("an SE published for another benchmark does NOT carry over",
      c0["band_basis"], "dispersion_published_for_another_benchmark")
check("  ...leaving the cell UNRESOLVED", c0["direction"], "UNRESOLVED")
...
f = direction(m)
check("identical scores are equivalent with no interval -- equality is not an inference",
      f["comparisons"][0]["direction"], "equivalent")
...
check("same cost / meaningful score gap outside a published CI -> the stronger side wins",
      direction(m)["comparisons"][0]["direction"], "codex")

# =====================================================================
# PRECEDENCE, COMPARABILITY AND EXCLUSION
# =====================================================================

# ------------------------------------------- vendor disagreement, precedence
m = base_meta()
m["comparability_groups"] = {
    "indep": group("model_intrinsic", "B", band=ci(2.0)),
    "vend": group("vendor_relative", "A", band=ci(2.0)),
}
m["records"] = [
...
m = base_meta()
m["comparability_groups"] = {"a": group("vendor_relative", "A", band=ci(2.0)),
                             "b": group("vendor_relative", "A", band=ci(2.0))}
m["records"] = [
...
# ------------------------------------------------------------- saturation
m = base_meta()
m["comparability_groups"] = {"g": group(saturated=True, band=ci(0.5))}
m["records"] = [row("a", "Fable 5.1", 85.02), row("b", "Astra", 87.27),
                row("c", "GPT-5.6 Sol", 85.77)]
check("a group declared saturated sets no direction even with a published CI",
      direction(m)["comparisons"][0]["direction"], "UNRESOLVED")

# --------------------------------------------------------- single ecosystem
m = base_meta()
m["comparability_groups"] = {"g": group(band=ci(1.0))}
...
m = base_meta()
m["comparability_groups"] = {"g": group(band=ci(1.0))}
m["excluded_from_direction"] = [{"benchmark": "B", "benchmark_version": None,
                                 "reason": "funded by a party"}]
m["records"] = [row("a", "Fable 5.1", 20.0), row("b", "Astra", 90.0), row("c", "GPT-5.6 Sol", 10.0)]
c0 = direction(m)["comparisons"][0]
check("a declared exclusion beats even a huge gap with a published CI",
      c0["direction"], "UNRESOLVED")
check("  ...and records why", c0["reason"].startswith("declared excluded:"), True)
check("  ...and contributes nothing to the capability tally",
      direction(m)["preferred"], "UNRESOLVED")

# ------------------------- two benchmarks in one source are NOT pooled
m = base_meta()
m["comparability_groups"] = {"g": group("vendor_relative", "C", band=ci(2.0))}
m["records"] = [
    ...
    row("f1", "Fable 5.1", 87.8, bench="FrontierMath", evidence_class="vendor_relative",
        source_tier="C"),
    row("f2", "Astra", 97.6, bench="FrontierMath", evidence_class="vendor_relative",
        source_tier="C"),
]
f = direction(m)
...
# ------------------- a two-row subset must not shrink the reported spread
m = base_meta()
m["comparability_groups"] = {"g": group(band=ci(2.0))}
...
check("a 1.9 gap stays equivalent inside a published 2.0 CI in a 2-model view",
      sub["cells"][0]["direction"], "equivalent")
full = direction(m)["comparisons"][0]
check("  ...and the spread diagnostic comes from the FULL cell, not the pair",
      full["observed_spread"], round(57.7 - 37.3, 4))
...
curve = C.compile_effort_curves(m["records"], m)[0]
check("a higher rung with no score gain and a higher cost is dominated, no band needed",
      [(d["rung"], d["dominated_by"], d["basis"]) for d in curve["dominated_rungs"]],
      [("max", "xhigh", "no_gain_at_higher_cost")])

# ------------- a small POSITIVE gain with no dispersion is not dominance
m = base_meta()
m["comparability_groups"] = {"g": group()}
m["records"] = [
    row("o1", "Opus 5", 50.0, effort="xhigh", cost_per_task_usd=4.88),
    row("o2", "Opus 5", 51.0, effort="max", cost_per_task_usd=5.86),
]
curve = C.compile_effort_curves(m["records"], m)[0]
check("a 1-point gain at higher cost is NOT called dominated without an interval",
      curve["dominated_rungs"], [])
check("  ...it is recorded as unresolved instead",
