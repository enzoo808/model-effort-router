...
| 9 | after the English port of `SKILL.md` + `reference.md`, cold re-run (4 parallel agents + a 2-eval re-run for two prompt/rubric tightenings) | **17/17**. n2's prompt was tightened to a mechanical enumeration (open-ended "review for security holes" splits cold agents D=1 vs D=3); 5b's expected moved to D=3 → Sol Ultra, and the SKILL.md inline example was corrected to match (a large defensive audit = adversarial hunting = D=3, consistent with §8) |
